...
function k_select_resource() {
  ...
  resource_type=$(cat ${resource_list} | fzf +m -0 --prompt="select a resource target: ")
...
function kac() {
  ...
  selection=$(k get ${resource_type} ${all_flag} | fzf --header-lines=1 --prompt="select resource type: ") || return
