...
1. **An agreed plan** — normally `deploy-plan.md` from `bw-release-planner`, whose steps
   ...
   **Read the `.md`, not the `.html`.** Both are GENERATED by
   `~/.claude/scripts/deploy-plan-render.mjs` from the planner's `deploy-plan.yaml`: the HTML
   is the human's copy (diagram, styling), the Markdown is yours. Because they come from one
   source they cannot disagree — if they appear to, the render is stale, so say so rather than
   picking whichever looks right. It opens with a checkbox
...
9. **Never write to the plan files.** `deploy-plan.yaml`, `deploy-plan.md` and
   `deploy-plan.html` belong to
...
`argocd vault`, `kargo ci prod`, `gh-pr 214`, a full URL, or `none` — derived by the renderer
from the plan's `watch:` field. **`none` means the object does not exist yet** (a PR not opened,
a workflow run not dispatched), not that nobody filled it in: there is nothing to resolve and
nothing to guess. Use it; it is the identifier read out of a real source rather than one you
